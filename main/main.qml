...
ApplicationWindow {
    ...
    // Новые свойства для передачи параметров
    ...
    function loadAllData() {
        ...
        function checkCompletion() {
            ...
            if (completed >= promises.length) {
                isLoading = false;
                if (teachers.length > 0 || students.length > 0 || groups.length > 0) {
        ...
        for (var i = 0; i < promises.length; i++) {
            promises[i].func(checkCompletion);
        }
    }
    ...
    function loadTeachers(callback) {
        ...
        if (!mainApi.initialized) {
            console.error("❌ API не инициализирован для загрузки преподавателей");
    ...
    function loadStudents(callback) {
        ...
        if (!mainApi.initialized) {
            console.error("❌ API не инициализирован для загрузки студентов");
            if (callback) callback();
            return;
        }
    ...
    function loadGroups(callback) {
        ...
        if (!mainApi.initialized) {
            console.error("❌ API не инициализирован для загрузки групп");
            if (callback) callback();
            return;
        }
    ...
    function loadPortfolio() {
        if (!mainApi.initialized) {
            console.error("❌ API не инициализирован для загрузки портфолио");
            return;
        }
    ...
    function loadEvents() {
        if (!mainApi.initialized) {
            console.error("❌ API не инициализирован для загрузки событий");
            return;
        }
    ...
    Rectangle {
        ...
        MainTitleBar {
            ...
            currentView: mainWindow.viewTitles[mainWindow.currentView] || "Главная панель"
        ...
        Rectangle {
            id: mainContent
            anchors { top: successMessage.bottom; bottom: parent.bottom; left: parent.left; right: parent.right; margins: 10; topMargin: 15 }
            color: "transparent"
            ...
            Rectangle {
                ...
                Loader {
                    ...
                    anchors.fill: parent
                    anchors.margins: 10
                    source: mainWindow.viewComponents[mainWindow.currentView] || "DashboardView.qml"
                }
            }
        }
        ...
        // Индикатор загрузки
        Rectangle {
            id: loadingOverlay; anchors.fill: mainContent; color: "#80000000"; radius: 12; visible: isLoading; z: 10
            Rectangle {
                width: 80; height: 80; radius: 40; color: "#40000000"; anchors.centerIn: parent
                RotationAnimation on rotation { from: 0; to: 360; duration: 1200; running: loadingOverlay.visible; loops: Animation.Infinite }
                Text { anchors.centerIn: parent; text: "⏳"; font.pixelSize: 24; color: "white" }
            }
            Column {
                anchors { horizontalCenter: parent.horizontalCenter; top: parent.verticalCenter; topMargin: 60 } spacing: 5
                Text { text: "Загрузка данных..."; font.pixelSize: 14; color: "white"; anchors.horizontalCenter: parent.horizontalCenter }
                Text { text: "Пожалуйста, подождите"; font.pixelSize: 11; color: "#cccccc"; anchors.horizontalCenter: parent.horizontalCenter }
            }
        }
    }
    ...
    Timer { id: errorAutoHideTimer; interval: 8000; onTriggered: showError("") }
    Timer { id: successAutoHideTimer; interval: 4000; onTriggered: showSuccess("") }
}
